        ...
        try:
            if response.status_code == 200:
                ...
                for class_name in category_classes_to_search:
                    category = soup.find_all(['a', 'div'], class_=class_name)
                    if category:
                        category_text = ','.join(
                            [i.text.replace('\n', ' ').replace('new', '') for i in category])
                        break

                offer_classes_to_search = ['font-weight-bold bigger', 'css-g9dzcj',
                                           'css-vb54bv', 'css-lr4g51', 'font-weight-bold']
                offer_text = 'No data'
                for class_name in offer_classes_to_search:
                    offer = soup.find_all(['h1', 'div'], class_=class_name)
                    if offer:
                        offer_text = ','.join(
                            [i.text.replace('\n', ' ') for i in offer])
                        break
                ...
                for class_name in salary_classes_to_search:
                    salary = soup.find_all('div', class_=class_name)
                    if salary:
                        salary_text = ','.join(
                            [i.text.replace('\n', ' ') for i in salary])
                        break
                ...
                for class_name in information_about_the_offer_to_search:
                    information_about_the_offer = soup.find_all(
                ...
                if not information_about_the_offer:
                    ...
                    if experience:
                        experience_text = ','.join(
                            [i.text.replace('\n', ' ') for i in experience])
                    operating_mode = soup.find_all(
                        'div', class_='tw-flex tw-items-center tw-w-full')
                    if operating_mode:
                ...
                for class_name in job_description_classes_to_search:
                    job_description = soup.find_all('div', class_=class_name)
                    if job_description:
                ...
                for class_name in application_form_classes_to_search:
                    application_form = soup.find_all('div', class_=class_name)
                    if application_form:
